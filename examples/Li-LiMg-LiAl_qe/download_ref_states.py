...
a = MPRester(api_key)
results = a.materials.search(
    material_ids=[f"mp-{ID}"], fields=["structure", "formula_pretty"]
)
results = results[0]
initial_structures = (
    results.structure
)  # .make_supercell([2, 2, 2]) only use this when the number of atoms is less than 2
