...
from pymatgen.analysis.molecule_matcher import IsomorphismMolAtomMapper, MoleculeMatcher
...
class Constraints(object):
    ...
    def __init__(self, constraints_parameters, composition_space):
        ...
        # parse the parameters and set to defaults if necessary
            ...
            elif constraints_parameters["min_lattice_length"] in (None, "default"):
                self.min_lattice_length = self.default_min_lattice_length
            else:
            ...
            elif constraints_parameters["max_lattice_length"] in (None, "default"):
                self.max_lattice_length = self.default_max_lattice_length
            else:
            ...
            elif constraints_parameters["min_lattice_angle"] in (None, "default"):
                self.min_lattice_angle = self.default_min_lattice_angle
            else:
            ...
            elif constraints_parameters["max_lattice_angle"] in (None, "default"):
                self.max_lattice_angle = self.default_max_lattice_angle
            else:
            ...
            elif constraints_parameters["allow_endpoints"] in (None, "default"):
                self.allow_endpoints = self.default_allow_endpoints
            else:
            ...
            elif constraints_parameters["per_species_mids"] in (None, "default"):
                self.set_all_mids_to_defaults(composition_space)
            else:
    ...
    def set_all_mids_to_defaults(self, composition_space):
        ...
        for i in range(0, len(elements)):
            for j in range(i, len(elements)):
                ...
                ] = self.default_mid_factor * (
                    elements[i].atomic_radius + elements[j].atomic_radius
                )
    ...
    def set_some_mids_to_defaults(self, composition_space):
        ...
        for i in range(0, len(elements)):
            for j in range(i, len(elements)):
                ...
                if (
                    test_key1 not in self.per_species_mids
                    and test_key2 not in self.per_species_mids
                ):
...
class Developer(object):
    ...
    def develop(self, organism, composition_space, constraints, geometry, pool):
        ...
        if not self.is_in_composition_space(
            organism, composition_space, constraints, pool
        ):
            return False
        ...
        if self.scale_density and len(pool.promotion_set) > 0 and organism.epa is None:
    ...
    def satisfies_num_atoms_constraints(self, organism, constraints):
        ...
        if len(organism.cell.sites) > constraints.max_num_atoms:
            print(
                "Organism {} failed max number of atoms constraint ".format(organism.id)
            )
            return False
        ...
        if len(organism.cell.sites) < constraints.min_num_atoms:
            print(
                "Organism {} failed min number of atoms constraint ".format(organism.id)
            )
            return False
        return True

    def is_in_composition_space(self, organism, composition_space, constraints, pool):
        """
        ...
        # for epa searches
        if composition_space.objective_function == "epa":
            return self.is_in_composition_space_epa(organism, composition_space)
        # for pd searches
        elif composition_space.objective_function == "pd":
            return self.is_in_composition_space_pd(
                organism, composition_space, constraints, pool
            )
    ...
    def is_in_composition_space_pd(
        self, organism, composition_space, constraints, pool
    ):
        """
        Returns a boolean indicating whether the organism is in the composition
        ...
        ) == len(composition_space.endpoints):
            print("Organism {} lies outside the composition space ".format(organism.id))
            return False
        ...
        if not constraints.allow_endpoints and len(pool.to_list()) > 0:
            for endpoint in composition_space.endpoints:
                if endpoint.almost_equals(organism.composition.reduced_composition):
                    print(
                        "Organism {} is at a composition space "
    ...
    def niggli_reduction(self, organism, geometry, constraints):
        ...
        if geometry.shape == "bulk":
            try:
                organism.cell.get_reduced_structure(reduction_algo="niggli")
            except:
                print(
                    "Niggli cell reduction failed on organism {} during "
            ...
            if not organism.cell.reduce_sheet_cell(geometry, constraints):
                print(
                    "2D Niggli cell reduction failed on organism {} "
    ...
    def scale_volume_epa(self, organism, pool):
        ...
        with warnings.catch_warnings():
            ...
            if str(organism.cell.lattice.a) == "nan" or organism.cell.lattice.a > 100:
                print(
                    "Volume scaling failed on organism {} during "
                    "development ".format(organism.id)
                )
                return False
        return True
        ...
        for i in range(len(comps)):
            for org in pool.promotion_set:
                if (comps[i].reduced_composition).almost_equals(
                    ...
                    vpa_mean += (org.cell.volume / len(org.cell.sites)) * fractions[i]
    ...
    def satisfies_lattice_constraints(self, organism, constraints):
        ...
        for angle in angles:
            if angle > constraints.max_lattice_angle:
                print(
                    "Organism {} failed max lattice angle "
                    "constraint ".format(organism.id)
                )
                return False
            elif angle < constraints.min_lattice_angle:
                print(
                    "Organism {} failed min lattice angle "
                    "constraint ".format(organism.id)
                )
                return False
        return True
            ...
            for species_symbol in species_symbols:
                ...
                for neighbor in neighbors:
                    if neighbor[0].specie.symbol == species_symbol:
                        print(
                            "Organism {} failed per-species minimum "
                            "interatomic distance constraint ".format(organism.id)
                        )
                        return False
        return True
...
class RedundancyGuard(object):
    ...
    def __init__(self, redundancy_parameters, geometry):
        ...
        # parse the parameters, and set to defaults if necessary
            ...
            elif redundancy_parameters["lattice_length_tol"] in (None, "default"):
                self.lattice_length_tol = self.default_lattice_length_tol
            else:
            ...
            elif redundancy_parameters["lattice_angle_tol"] in (None, "default"):
                self.lattice_angle_tol = self.default_lattice_angle_tol
            else:
                self.lattice_angle_tol = redundancy_parameters["lattice_angle_tol"]
            ...
            elif redundancy_parameters["use_primitive_cell"] in (None, "default"):
                self.use_primitive_cell = self.default_use_primitive_cell
            else:
            ...
            elif redundancy_parameters["attempt_supercell"] in (None, "default"):
                self.attempt_supercell = self.default_attempt_supercell
            else:
        ...
        self.structure_matcher = StructureMatcher(
            self.lattice_length_tol,
            self.site_tol,
            self.lattice_angle_tol,
            self.use_primitive_cell,
            False,
        ...
        if geometry.shape == "cluster" or geometry.shape == "wire":
            ...
            self.molecule_matcher = MoleculeMatcher(self.rmsd_tol, iso_mol_atom_mapper)
            ...
            for organism in orgs_list:
                if new_organism.id != organism.id:  # just in case
                    ...
                    if self.check_structures(new_organism, organism, geometry):
                        print(
                            "Organism {} failed structural redundancy - "
            ...
            for organism in orgs_list:
                if new_organism.id != organism.id and organism.epa is not None:
                    ...
                    if abs(new_organism.epa - organism.epa) < self.epa_diff:
                        print(
                            "Organism {} failed energy per atom redundancy "
                            "- looks like organism {} ".format(
                                new_organism.id, organism.id
                            )
                        )
