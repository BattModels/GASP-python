...
def make_objects(parameters):
    ...
    if "Constraints" in parameters:
        if "min_num_atoms" in parameters["Constraints"]:
            if parameters["Constraints"]["min_num_atoms"] < 2:
                print(
                    'The value passed to the "min_num_atoms" keyword in the '
                    "Constraints block must greater than or equal to 2."
                )
                print("Quitting...")
                quit()
        constraints = development.Constraints(
            parameters["Constraints"], composition_space
        )
    else:
    ...
    if len(composition_space.endpoints) > 1:
        ...
        atoms_per_comp = composition_space.endpoints[0].reduced_composition.num_atoms
    ...
    # make the variations
    variations_list = make_variations(
        parameters, default_variation_fractions, composition_space
    )

    # check that at least one variation has been used
    if len(variations_list) == 0:
        print(
            "At least one variation must be used. Either leave entire "
        ...
        if "num_promoted" in parameters["Pool"]:
            if parameters["Pool"]["num_promoted"] < 1:
                ...
                pool = population.Pool(
                    parameters["Pool"], composition_space, run_dir_name
                )
        else:
            pool = population.Pool(parameters["Pool"], composition_space, run_dir_name)
    ...
    if "CompositionFitnessWeight" not in parameters:
        ...
        if "max_weight" in parameters["CompositionFitnessWeight"]:
            if (
                parameters["CompositionFitnessWeight"]["max_weight"] < 0
                or parameters["CompositionFitnessWeight"]["max_weight"] > 1
            ):
                print(
                    "The maximum weight of the composition fitness must lie"
                    " in the interval [0,1]."
                )
                print(
                    'Please change the value passed to the "max_weight" '
...
def make_organism_creators(parameters, composition_space, constraints):
    ...
    # make the specified creators
        ...
        if (
            "random" not in parameters["InitialPopulation"]
            and "from_files" not in parameters["InitialPopulation"]
        ):
            print(
                "At least one valid option for making structures for the "
                "initial population must be provided."
            )
            print(
                'Please use the "random" and/or "from_files" keywords in '
        ...
        if "random" in parameters["InitialPopulation"]:
            random_organism_creator = organism_creators.RandomOrganismCreator(
                parameters["InitialPopulation"]["random"],
                composition_space,
                constraints,
            )
        ...
        if "from_files" not in parameters["InitialPopulation"]:
            if composition_space.objective_function == "pd":
                print(
                    "For phase diagram searches, reference structures at "
                    "each endpoint of the composition space must be "
                    "provided."
                )
                print(
                    'Please use the "from_files" keyword in the '
            ...
            print(
                "Restarting from the given database. Only proceed if this is intended."
            )
        # if nothing is given after the from_files keyword
        elif parameters["InitialPopulation"]["from_files"] is None:
            print(
                "The path to the folder containing the files must be "
        ...
        # if path_to_folder keyword is not given
            ...
            print(
                'Incorrect keyword given after "from_files" in the '
            ...
            if given_path is None:
                print(
                    "The path to the folder containing the files for the "
            ...
            # if the given path does not exist
                ...
                print(
                    "The given folder containing structures for the initial "
            ...
            # if the folder exists, check that it contains files
                ...
                if composition_space.objective_function == "pd":
                    ...
                    for endpoint in composition_space.endpoints:
                        for cell in cells:
                            if (
                                cell.composition.reduced_composition.almost_equals(
                                    endpoint.reduced_composition
                                )
                                and endpoint not in provided_endpoints
                            ):
                    ...
                    for endpoint in composition_space.endpoints:
                        if endpoint not in provided_endpoints:
                            print(
                                "Error: valid structure files not provided "
...
def make_default_organism_creator(composition_space, constraints):
    ...
    if composition_space.objective_function == "pd":
        print(
            "For phase diagram searches, reference structures at each "
...
def make_energy_calculator(parameters, geometry, composition_space):
    ...
    if "EnergyCode" not in parameters:
        print(
            "A method for calculating energy must be provided. Please use "
            'the "EnergyCode" keyword.'
        )
        print("Quitting...")
        quit()
    elif parameters["EnergyCode"] is None:
        print('An energy code must be specified after the "EnergyCode" ' "keyword.")
        print("Quitting...")
        quit()
    # for GULP
    ...
    # for VASP
        ...
        return make_vasp_energy_calculator(parameters, composition_space, geometry)
    elif "quantum_espresso" in parameters["EnergyCode"]:
        return make_quantum_espresso_energy_calculator(parameters, geometry)
    else:
...
def make_gulp_energy_calculator(parameters, geometry):
    ...
    if parameters["EnergyCode"]["gulp"] is None:
        print(
            "No GULP header or potential files given. Please use the "
        ...
        if "header_file" not in parameters["EnergyCode"]["gulp"]:
            print(
                "A GULP header file must be provided. Please use the "
                '"header_file" keyword.'
            )
            print("Quitting...")
            quit()
            ...
            print(
                'No GULP header file given after the "header_file" '
        ...
        if "potential_file" not in parameters["EnergyCode"]["gulp"]:
            print(
                "A GULP potential file must be provided. Please use the "
            ...
            print(
                "No GULP potential file given after the "
...
def make_lammps_energy_calculator(parameters, geometry):
    ...
    if parameters["EnergyCode"]["lammps"] is None:
        print('No LAMMPS input script given. Please use the "input_script" ' "keyword.")
        print("Quitting...")
        quit()
    else:
        ...
        if "input_script" not in parameters["EnergyCode"]["lammps"]:
            print(
                "A LAMMPS input script must be provided. Please use the "
                '"header_file" keyword.'
            )
            print("Quitting...")
            quit()
            ...
            print(
                'No LAMMPS input script given after the "input_script" '
                "keyword. Please provide one."
            )
            print("Quitting...")
            quit()
        ...
        return energy_calculators.LammpsEnergyCalculator(input_script_path, geometry)
...
def make_vasp_energy_calculator(parameters, composition_space, geometry):
    ...
    if parameters["EnergyCode"]["vasp"] is None:
        ...
        if "incar" not in parameters["EnergyCode"]["vasp"]:
            print('An INCAR file must be provided. Please use the "incar" ' "keyword.")
            print("Quitting...")
            quit()
        elif parameters["EnergyCode"]["vasp"]["incar"] is None:
            print(
                'No INCAR file was given after the "incar" keyword. Please '
                "provide one."
            )
            print("Quitting...")
            quit()
        ...
        if "kpoints" not in parameters["EnergyCode"]["vasp"]:
            print(
                "A KPOINTS file must be provided. Please use the " '"kpoints" keyword.'
            )
            print("Quitting...")
            quit()
        elif parameters["EnergyCode"]["vasp"]["kpoints"] is None:
            print(
                'No KPOINTS file was given after the "kpoints" keyword. '
                "Please provide one."
            )
            print("Quitting...")
            quit()
        ...
        if "potcars" not in parameters["EnergyCode"]["vasp"]:
            print(
                "POTCAR file(s) must be provided. Please use the " '"potcars" keyword.'
            )
            print("Quitting...")
            quit()
        elif parameters["EnergyCode"]["vasp"]["potcars"] is None:
            print(
                'No POTCAR files were given after the "potcars" keyword. '
                "Please provide them."
            )
            print("Quitting...")
            quit()
            ...
            if len(potcar_paths) < len(elements_list):
                print(
                    "Not enough POTCAR files provided - one must be "
            ...
            for element in elements_list:
                if element.symbol not in potcar_paths:
                    print(
                        "No POTCAR file given for {}. Please provide "
...
def make_quantum_espresso_energy_calculator(parameters, geometry):
    ...
    if parameters["EnergyCode"]["quantum_espresso"] is None:
        ...
        if "qe_calc_settings" not in parameters["EnergyCode"]["quantum_espresso"]:
            print(
                'An qe_calc_settings file must be provided. Please use the "qe_calc_settings" '
                "keyword."
            )
            print("Quitting...")
            quit()
            ...
            print(
                'No qe_calc_settings file was given after the "qe_calc_settings" keyword. Please '
                "provide one."
            )
            print("Quitting...")
            quit()
            ...
            # get the path to the INCAR file
            qe_calc_setting_path = parameters["EnergyCode"]["quantum_espresso"][
                "qe_calc_settings"
            ]
            slurm_template_path = parameters["EnergyCode"]["quantum_espresso"][
                "slurm_template_path"
            ]
...
def make_stopping_criteria(parameters, composition_space):
    ...
    if "StoppingCriteria" not in parameters:
        ...
        if parameters["StoppingCriteria"]["found_structure"] in (None, "default"):
            ...
            # check that the file has the correct suffix or prefix
                ...
                os.path.basename(given_path).endswith(".cif")
                or os.path.basename(given_path).startswith("POSCAR.")
            ):
                print(
                    "File containing structure to find must be in POSCAR or "
...
def make_variations(parameters, default_fractions, composition_space):
    ...
    if "Variations" not in parameters:
        ...
        if "Mating" not in parameters["Variations"]:
            ...
            print(
                'If the "Mating" keyword is used, its "fraction" keyword '
                "must also be set."
            )
            print("Quitting...")
            quit()
        else:
            if parameters["Variations"]["Mating"]["fraction"] in (None, "default"):
                print(
                    'The "fraction" kwyword is not optional and must '
        ...
        if "StructureMut" not in parameters["Variations"]:
            ...
            print(
                'If the "StructureMut" keyword is used, its "fraction" '
            ...
            if parameters["Variations"]["StructureMut"]["fraction"] in (
                ...
                print(
                    'The "fraction" keyword is not optional and must '
        ...
        if "NumAtomsMut" not in parameters["Variations"]:
            ...
            print(
                'If the "NumAtomsMut" keyword is used, its "fraction" '
                "keyword must also be set."
            )
            print("Quitting...")
            quit()
        else:
            if parameters["Variations"]["NumAtomsMut"]["fraction"] in (None, "default"):
                print(
                    'The "fraction" keyword is not optional and must '
                    "contain a valid entry (between 0 and 1) for the "
        ...
        if "Permutation" not in parameters["Variations"]:
            ...
            print(
                'If the "Permutation" keyword is used, its "fraction" '
                "keyword must also be set."
            )
            print("Quitting...")
            quit()
        else:
            if parameters["Variations"]["Permutation"]["fraction"] in (None, "default"):
                print(
                    'The "fraction" keyword is not optional and must '
                    "contain a valid entry (between 0 and 1) for the "
...
def make_default_variations(default_fractions, composition_space):
    ...
    permutation = variations.Permutation(
        {"fraction": default_fractions["permutation"]}, composition_space
    )
    variations_list.append(mating)
    variations_list.append(structure_mut)
    variations_list.append(num_atoms_mut)
