...
class RandomOrganismCreator(object):
    ...
    def __init__(self, random_org_parameters, composition_space, constraints):
        ...
        if composition_space.objective_function == "epa":
            ...
            num_formulas = target_number / composition_space.endpoints[0].num_atoms
            if num_formulas < 2:
                min_of_max = int(2 * composition_space.endpoints[0].num_atoms)
            else:
                min_of_max = int(
                    round(num_formulas) * composition_space.endpoints[0].num_atoms
                )
        else:
        ...
        # parse the parameters and set to defaults if necessary
            ...
            if "max_num_atoms" not in random_org_parameters:
                ...
                self.max_num_atoms = self.default_max_num_atoms
            elif random_org_parameters["max_num_atoms"] > constraints.max_num_atoms:
                print(
                    'The value passed to the "max_num_atoms" keyword in the '
                ...
                print("Quitting...")
                quit()
            elif random_org_parameters["max_num_atoms"] < constraints.min_num_atoms:
                print(
                    'The value passed to the "max_num_atoms" keyword in the '
            ...
            if "volumes_per_atom" not in random_org_parameters:
                pass
            elif random_org_parameters["volumes_per_atom"] in (None, "default"):
                pass
            else:
                # replace the specified volumes per atom with the given values
                for symbol in random_org_parameters["volumes_per_atom"]:
                    self.vpas[symbol] = random_org_parameters["volumes_per_atom"][
                        symbol
                    ]
    ...
    def get_default_vpas(self, composition_space):
        ...
        for element in composition_space.get_all_elements():
            default_vpas[element.symbol] = self.all_default_vpas[element.symbol]
        return default_vpas

    def create_organism(self, id_generator, composition_space, constraints, random):
        """
        ...
        random_org = Organism(random_cell, id_generator, self.name, composition_space)
    ...
    def get_species_list(self, composition_space, constraints, random):
        ...
        """
        if composition_space.objective_function == "epa":
            return self.get_epa_species_list(composition_space, constraints, random)
        elif composition_space.objective_function == "pd":
            return self.get_pd_species_list(composition_space, constraints, random)
    ...
    def get_epa_species_list(self, composition_space, constraints, random):
        ...
        max_num_formulas = int(math.floor(self.max_num_atoms / num_atoms_in_formula))
        min_num_formulas = int(
            math.ceil(constraints.min_num_atoms / num_atoms_in_formula)
        )
    ...
    def get_pd_species_list(self, composition_space, constraints, random):
        ...
        for element in element_amounts:
            ...
            ).limit_denominator(self.max_num_atoms)
        ...
        if num_atoms > self.max_num_atoms:
            numerator = random.randint(
                int(round(0.5 * (constraints.min_num_atoms + self.max_num_atoms))),
                self.max_num_atoms,
            )
            factor = numerator / num_atoms
            for element in element_amounts:
                element_amounts[element] = round(factor * element_amounts[element])
        ...
        if not self.allow_endpoints:
            for endpoint in composition_space.endpoints:
                if endpoint.almost_equals(random_composition.reduced_composition):
                    return None
    ...
    def get_random_endpoint_fractions(self, composition_space, random):
        ...
        for i in range(1, len(bindary_edges)):
            weighted_average = np.add(
                weighted_average, random.random() * bindary_edges[i]
            )
        ...
        if endpoint_fracs[0] < 0:
            return self.get_random_endpoint_fractions(composition_space, random)
        else:
    ...
    def scale_volume(self, random_cell):
        ...
        with warnings.catch_warnings():
            ...
            if str(random_cell.lattice.a) == "nan" or random_cell.lattice.a > 100:
                return False
            else:
                return True
    ...
    def update_status(self):
        ...
        print(
            "Organisms left for {}: {} ".format(self.name, self.number - self.num_made)
        )
...
class DatabaseOrganismCreator(object):
    ...
    def create_organism(self, id_generator, composition_space, constratins, random):
        ...
        new_org = Organism(new_cell, id_generator, self.name, composition_space)
        print(
            "Making organism {} from database: garun_index = {}".format(
...
class FileOrganismCreator(object):
    ...
    def __init__(self, path_to_folder):
        ...
        self.files = [
            ...
            if os.path.isfile(os.path.join(self.path_to_folder, f))
        ]
        ...
        if self.files[self.num_made - 1].endswith(".cif") or self.files[
            ...
            try:
                new_cell = Cell.from_file(
                    str(self.path_to_folder) + "/" + str(self.files[self.num_made - 1])
                )
                new_org = Organism(new_cell, id_generator, self.name, composition_space)
                print(
                    "Making organism {} from file: {} ".format(
                ...
                print(
                    "Error reading structure from file: {} ".format(
            ...
            print(
                "File {} has invalid extension - file must end with .cif or "
