...
def print_parameters(objects_dict):
    ...
    with open(os.getcwd() + "/ga_parameters", "w") as parameters_file:
        ...
        if energy_calculator.name == "gulp":
            parameters_file.write(
                "        header_file: " + energy_calculator.header_path + "\n"
            )
            parameters_file.write(
                "        potential_file: " + energy_calculator.potential_path + "\n"
            )
        elif energy_calculator.name == "lammps":
            parameters_file.write(
                "        input_script: " + energy_calculator.input_script + "\n"
            )
        elif energy_calculator.name == "vasp":
            parameters_file.write(
                "        incar: " + energy_calculator.incar_file + "\n"
            )
            parameters_file.write(
                "        kpoints: " + energy_calculator.kpoints_file + "\n"
            )
        ...
        for creator in organism_creators:
            if creator.name == "random organism creator":
                ...
                parameters_file.write(
                    "        max_num_atoms: " + str(creator.max_num_atoms) + "\n"
                )
                parameters_file.write(
                    "        allow_endpoints: " + str(creator.allow_endpoints) + "\n"
                )
                parameters_file.write("        volumes_per_atom: " + "\n")
                for vpa in creator.vpas:
                    parameters_file.write(
                        "            " + str(vpa) + ": " + str(creator.vpas[vpa]) + "\n"
                    )
            elif creator.name == "file organism creator":
                parameters_file.write("    from_files: \n")
                parameters_file.write("        number: " + str(creator.number) + "\n")
                parameters_file.write(
                    "        path_to_folder: " + str(creator.path_to_folder) + "\n"
                )
        parameters_file.write("\n")
        ...
        parameters_file.write(
            "    num_parents: " + str(pool.selection.num_parents) + "\n"
        )
        ...
        if composition_space.objective_function == "pd":
            ...
            parameters_file.write(
                "    max_weight: " + str(pool.comp_fitness_weight.max_weight) + "\n"
            )
            parameters_file.write(
                "    power: " + str(pool.comp_fitness_weight.power) + "\n"
            )
            parameters_file.write("\n")
        ...
        for variation in variations:
            if variation.fraction == 0:
                ...
                if variation.name == "mating":
                    ...
                    parameters_file.write(
                        "        fraction: " + str(variation.fraction) + "\n"
                    )
                    parameters_file.write(
                        "        mu_cut_loc: " + str(variation.mu_cut_loc) + "\n"
                    )
                    parameters_file.write(
                        "        sigma_cut_loc: " + str(variation.sigma_cut_loc) + "\n"
                    )
                    parameters_file.write(
                        "        shift_prob: " + str(variation.shift_prob) + "\n"
                    )
                    parameters_file.write(
                        "        rotate_prob: " + str(variation.rotate_prob) + "\n"
                    )
                    parameters_file.write(
                        "        doubling_prob: " + str(variation.doubling_prob) + "\n"
                    )
                    parameters_file.write(
                        "        grow_parents: " + str(variation.grow_parents) + "\n"
                    )
                    parameters_file.write(
                        "        merge_cutoff: " + str(variation.merge_cutoff) + "\n"
                    )
                    ...
                    parameters_file.write(
                        "        mu_num_adds: " + str(variation.mu_num_adds) + "\n"
                    )
                    parameters_file.write(
                        "        sigma_num_adds: "
                    ...
                    parameters_file.write(
                        "        scale_volume: " + str(variation.scale_volume) + "\n"
                    )
                    ...
                    parameters_file.write(
                        "        mu_num_swaps: " + str(variation.mu_num_swaps) + "\n"
                    )
                    parameters_file.write(
                        "        sigma_num_swaps: "
        ...
        parameters_file.write(
            "    scale_density: " + str(developer.scale_density) + "\n"
        )
        parameters_file.write("\n")
        ...
        parameters_file.write(
            "    min_num_atoms: " + str(constraints.min_num_atoms) + "\n"
        )
        parameters_file.write(
            "    max_num_atoms: " + str(constraints.max_num_atoms) + "\n"
        )
        parameters_file.write(
            "    min_lattice_length: " + str(constraints.min_lattice_length) + "\n"
        )
        parameters_file.write(
            "    max_lattice_length: " + str(constraints.max_lattice_length) + "\n"
        )
        parameters_file.write(
            "    min_lattice_angle: " + str(constraints.min_lattice_angle) + "\n"
        )
        parameters_file.write(
            "    max_lattice_angle: " + str(constraints.max_lattice_angle) + "\n"
        )
        parameters_file.write(
            "    allow_endpoints: " + str(constraints.allow_endpoints) + "\n"
        )
        ...
        for pair in constraints.per_species_mids:
            parameters_file.write(
                ...
                + pair
                + ": "
                + str(float(constraints.per_species_mids[pair]))
                + "\n"
            )
        parameters_file.write("\n")
        ...
        parameters_file.write(
            "    lattice_length_tol: " + str(redundancy_guard.lattice_length_tol) + "\n"
        )
        parameters_file.write(
            "    lattice_angle_tol: " + str(redundancy_guard.lattice_angle_tol) + "\n"
        )
        parameters_file.write("    site_tol: " + str(redundancy_guard.site_tol) + "\n")
        parameters_file.write(
            "    use_primitive_cell: " + str(redundancy_guard.use_primitive_cell) + "\n"
        )
        parameters_file.write(
            "    attempt_supercell: " + str(redundancy_guard.attempt_supercell) + "\n"
        )
        parameters_file.write("    rmsd_tol: " + str(redundancy_guard.rmsd_tol) + "\n")
        ...
        if stopping_criteria.epa_achieved is not None:
            parameters_file.write(
                "    epa_achieved: " + str(stopping_criteria.epa_achieved) + "\n"
            )
